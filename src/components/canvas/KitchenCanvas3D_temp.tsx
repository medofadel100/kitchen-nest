...
export const KitchenCanvas3D = ({ readOnly = false }: { readOnly?: boolean }) => {
  ...
  // Ù„Ø¥ØºÙ„Ø§Ù‚ Ø§Ù„Ù‚Ø§Ø¦Ù…Ø© Ø¹Ù†Ø¯ Ø§Ù„Ø¶ØºØ· ÙÙŠ Ø£ÙŠ Ù…ÙƒØ§Ù†
    ...
    >
      ...
      {selectedElement?.type === 'unit' && !readOnly && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 bg-zinc-950/80 backdrop-blur-xl border border-zinc-800 p-2 rounded-2xl shadow-2xl">
          ...
          >
            <Scaling size={16} /> ØªØºÙŠÙŠØ± Ø§Ù„Ù…Ù‚Ø§Ø³ (Stretch)
          </button>
          ...
          >
            <RefreshCw size={16} /> Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„ÙƒØ§Ù…ÙŠØ±Ø§
          </button>
        </div>
      )}
      ...
      >
        ...
        {units.filter(u => !u.isHidden).map((unit) => {
          ...
          // Ø§Ù„Ù„ÙˆÙ† Ù…Ù† Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„ÙˆØ­Ø¯Ø© Ù…Ø¨Ø§Ø´Ø±Ø© â€” Ø§Ù„Ù„ÙˆÙ† Ø§Ù„Ù…Ø®ØªØ§Ø± Ù…Ù† Ø§Ù„Ù€ palette
          ...
          const isFridge = unit.label?.includes('Ø«Ù„Ø§Ø¬Ø©') || unit.label?.toLowerCase().includes('fridge');
          const isFreezer = unit.label?.includes('ÙØ±ÙŠØ²Ø±') || unit.label?.toLowerCase().includes('freezer');
          const isOven = unit.label?.includes('ÙØ±Ù†') || unit.label?.toLowerCase().includes('oven') || unit.label?.includes('Ø¨ÙˆØªØ§Ø¬Ø§Ø²');
          const isStove = unit.label?.includes('Ø¨ÙˆØªØ§Ø¬Ø§Ø²') || unit.label?.toLowerCase().includes('stove') || unit.label?.toLowerCase().includes('hob');
          const isDishwasher = unit.label?.includes('ØºØ³Ø§Ù„Ø© Ø£Ø·Ø¨Ø§Ù‚') || unit.label?.toLowerCase().includes('dishwasher');
          const isWashingMachine = unit.label?.includes('ØºØ³Ø§Ù„Ø© Ù…Ù„Ø§Ø¨Ø³') || unit.label?.toLowerCase().includes('washing_machine');
          const isDryer = unit.label?.includes('Ù…Ø¬ÙÙ') || unit.label?.toLowerCase().includes('dryer');
          const isSink = unit.label?.includes('Ø­ÙˆØ¶') || unit.label?.toLowerCase().includes('sink');
          ...
          const getVariant = (): 'standard' | 'premium' | 'compact' => {
            ...
            if (label.includes('premium') || label.includes('Ø¨Ø±ÙŠÙ…ÙŠÙˆÙ…') || label.includes('ÙØ§Ø®Ø±Ø©')) return 'premium';
            if (label.includes('compact') || label.includes('ÙƒÙˆÙ…Ø¨Ø§ÙƒØª') || label.includes('Ù…Ø¯Ù…Ø¬')) return 'compact';
            return 'standard';
          };
      ...
      {contextMenu && (
        ...
        >
          {contextMenu.type === 'unit' && (
            ...
            >
              {(() => {
                ...
                return targetUnit?._3dDoorOpen ? 'ðŸ”’ Ø¥ØºÙ„Ø§Ù‚ Ø§Ù„Ø£Ø¨ÙˆØ§Ø¨' : 'ðŸ”“ ÙØªØ­ Ø§Ù„Ø£Ø¨ÙˆØ§Ø¨';
              })()} 
              <EyeOff size={16} />
            </button>
          )}
          ...
          <button
            ...
            className="w-full px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-white flex items-center justify-end gap-3 transition-colors"
          >
            Ø¥Ø®ÙØ§Ø¡ / Ø¥Ø¸Ù‡Ø§Ø± <EyeOff size={16} />
          </button>
          ...
          <button
            ...
            className="w-full px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-white flex items-center justify-end gap-3 transition-colors"
          >
            ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ù…Ù‚Ø§Ø³Ø§Øª <Edit2 size={16} />
          </button>
